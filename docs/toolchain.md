...
| IANA time zone database | `third_party/tzdata/` (release tarball and `.asc`), compiled by `zic` and embedded by `tools/tzdata/generate.py`; asserted against `zdump` at build time by `tzcheck` | **not yet vendored**: the release, its SHA-256 and the signature verification go here when it lands; the binary carries the placeholder until then. zic and zdump come from the build host (Ubuntu: `libc-bin`, version printed by `zic --version`); they are generation tools, nothing of theirs ships |
