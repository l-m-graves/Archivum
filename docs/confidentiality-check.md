...
| Punchline free text (`time_entries.note`, `correction_reason`, `approvals.note`, `exceptions.detail`) | An employee's or supervisor's words in the feed or a log line | Not declared recordable: presence and shape only in every image; never in a log line (the sync test plants a canary note and scans the feed and the audit log); returned only to callers in scope | `punchline::Module::extend_policy` omits them; `tests/server/server_punchline_test.cpp` |
